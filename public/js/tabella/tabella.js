...
Tabella.prototype.move = function(x) {
    ...
    if (x === 'right') {
        if (Animator.animate(self.slidingRows, cellWidth, self.options.duration)){
            self.pointer++;
        }
    } else {
        if (x === 'left') {
            ...
            if (typeof x === 'number') {
                if (Animator.animate(self.slidingRows, x, getReboundTime(x, self.options.reboundSpeed))){
                    self.pointer = x/cellWidth;
                }
            } else {
